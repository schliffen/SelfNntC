            ...
            if (strcmp(buffer_layer_type, "Conv") == 0) {
                ...
                if (kernel_height != 0 && kernel_width != 0 && num_kernels != 0) {
                    ...
                    for (kernel = 0; kernel < num_kernels; kernel++) {
                        if (fread((void *) values, sizeof(float), kernel_height * kernel_width, binary_file) !=
                ...
                //adding PRelu
            } else if (strcmp(buffer_layer_type, "PRelu") == 0) {
                //todo: complete here...
                uint32_t kernel_height = 0;
                uint32_t kernel_width = 0;
                uint32_t num_kernels = 0;
...
//                        printf("%.6f\n", *(*kernels)[kernel_idx][kernel]);
            ...
            }else if(strcmp(buffer_layer_type, "BatchNormalization1d") == 0){
            //todo: complete here...
                // read gamma values
