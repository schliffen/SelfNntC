...
void concatenate_2D_naive(fp_t** input_channels, const uint16_t** input_shape,
                    const uint16_t dimension, const uint16_t num_inputs, fp_t* output_channel) {
...
//    uint16_t input_channel_size = width * height;
    ...
    if(dimension == 0) {
        ...
        for(input_channel = 0; input_channel < num_inputs; input_channel++){
            uint16_t input_channel_size = input_shape[input_channel][0] * input_shape[input_channel][1];

            memcpy(&output_channel[input_channel* input_channel_size],
                   input_channels[input_channel],input_channel_size * sizeof(fp_t));
        }
    }
...
//            for(input_channel = 0; input_channel < num_inputs; input_channel++) {
//
//                input_channel_size = input_shape[input_channel][1] * input_shape[input_channel][2];
//
////                memcpy(&output_channel[input_channel][output_position_counter],
////                       input_channels[input_channel][input_channel],
////                       input_channel_size * sizeof(fp_t));
//
...
void concatenate_2D_output(fp_t*** input_channels, const uint16_t** input_shape,
                          const uint16_t dimension, const uint16_t num_inputs, fp_t** output_channel) {
...
//    uint16_t input_channel_size = width * height;
    ...
    // probably the same as flatten
    if(dimension == 0) {
        for(int num_in = 0; num_in < num_inputs; num_in++) {
            uint16_t input_channel_size = input_shape[num_in][0] ;

            for (int dim1 = 0; dim1 < input_channel_size; dim1++) {
                global_dim +=dim1;
//                output_channel[global_dim][0] = input_channels[num_in][global_dim ][0];
//                output_channel[global_dim][1] = input_channels[num_in][global_dim][1];
//                output_channel[global_dim][2] = input_channels[num_in][global_dim][2];
//                output_channel[global_dim][3] = input_channels[num_in][global_dim][3];
                memcpy(&output_channel[global_dim],
                       &input_channels[num_in][global_dim], input_shape[num_in][1] * sizeof(fp_t));
            }
        }
    }
