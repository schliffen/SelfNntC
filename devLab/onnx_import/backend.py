...
class BackendRep(backend_base.BackendRep):
    ...
    def _remove_nops(self, graph, constant_states):
        ...
        for node in list(graph.nodes):
            ...
            if node.op_type == "Reshape":
                ...
                if input_state.value is None and reshape_state.shape is None:
                    is_nop = True
            ...
            if node.op_type == "Concat":
                ...
                is_nop = False
                # todo: update this part for different casts
                if node.attrs['axis'] == 0 or nomin_state.shape is None or denom_state.shape is None: # or nomin_state.value is None or denom_state.value is None :
                    is_nop = True
    ...
    def _generate_parameters(self, graph, memory_manager):
        ...
        parameter_header = "#ifndef "  + naming_prefix.upper() +  "NETWORK_PARAMETERS_H\n"
        parameter_header += "#define " + naming_prefix.upper() +  "NETWORK_PARAMETERS_H\n"
    ...
    def _generate_weights_file(self, graph):
        ...
        ops_to_ignore = ['Reshape', 'Mul', 'PRelu']
    ...
    def _generate_network_initialization(self, graph, memory_manager):
        ...
        initialization_header = "#ifndef " + naming_prefix.upper() + "NETWORK_INITIALIZATION_H\n"
        initialization_header += "#define " + naming_prefix.upper() + "NETWORK_INITIALIZATION_H\n"
        initialization_header += "#include <stdlib.h>\n"
        initialization_header += "#include <stdint.h>\n"
        initialization_header += "#include "  + naming_prefix +  "\"ai-cnn/parameters.h\"\n\n"
        ...
        initialization_header += "#endif // "   + naming_prefix.upper() +  "NETWORK_INITIALIZATION_H\n"
    ...
    def _generate_network_cleanup(self, graph, memory_manager):
        ...
        :return:
        """
        cleanup_header = "#ifndef " + naming_prefix.upper() + "NETWORK_CLEANUP_H\n"
        cleanup_header += "#define " + naming_prefix.upper() + "NETWORK_CLEANUP_H\n"
        ...
        cleanup_header += "#endif // "  + naming_prefix.upper() +  "NETWORK_CLEANUP_H\n"
    ...
    def _select_implementations(self, graph, memory_manager, constant_states, naming_prefix):
        """
        ...
        for node in graph.nodes:
            ...
            for op in OperationRegistry.get_ops(node.op_type):
                candidate = op.create(node, graph, memory_manager, constant_states, naming_prefix)
    ...
    def _export_model(self):
        ...
        implementations = self._select_implementations(graph, memory_manager, constant_states, naming_prefix)
        ...
        """Currently we only allow single input (no batch processing) to the CNN, but this may be -channel input"""
        inputs = graph.inputs
        if len(inputs) > 1:
            ...
            if len(input_shape) == 4:
                ...
                input_defs = ["fp_t ** "+ naming_prefix + n for n in input_names]
        ...
        for output in outputs:
            ...
            if len(output_shape) == 2:
                ...
                output_defs = ["fp_t * " + naming_prefix + output.name]
        ...
        network_header = "#ifndef " + naming_prefix.upper() + "NETWORK_H\n"
        network_header += "#define " + naming_prefix.upper() + "NETWORK_H\n\n"
        ...
        network_header += "#endif //" + naming_prefix + "NETWORK_H\n"
    ...
    def save(self, folder):
        ...
        with open(os.path.join(folder, naming_prefix + "Makefile"), "w") as f:
            f.write(self.makefile)

        with open(os.path.join(folder, naming_prefix + "dummy_input.c"), "w") as f:
            f.write(self.dummy_input)

        with open(os.path.join(folder, naming_prefix + "reference_input.c"), "w") as f:
