...
void test_relu_naive(int32_t error[]) {
    ...
    fp_t expected_relu[input_width] = {9,11, 0, 0, 0, 0, 0, 5, 0, 7};
    fp_t expected_prelu[input_width]  = { 9.,   11. ,  -1. ,  -1.25 ,-2.25 ,-1.,   -1.75 , 5.  ,  0.   , 7.  };

    fp_t* reluout = (fp_t*) malloc(input_width * input_height * sizeof(fp_t));

    relu_naive(input, input_height, input_width, reluout);

    fp_t* preluout = (fp_t*) malloc(input_width * input_height * sizeof(fp_t));
    prelu( input,  input_height, input_width, preluout, .25 );

    //error++;
    int32_t error1 = compare1dFloatArray(expected_relu, reluout, input_width, .0001);
    printf("relu err: %d \n", error1 );
    error[0] = error1;
//    error++;
    error1 = compare1dFloatArray(expected_prelu, preluout, input_width, .00001);
    printf("relu err: %df \n", error1 );
    error[1] = error1;
//    error -=2;
